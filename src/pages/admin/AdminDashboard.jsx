...
const AdminDashboard = () => {
  ...
  const formatStatus = (status) => {
    if (!status) return { label: 'Unknown', classes: 'bg-[#111111] text-zinc-300 border-[#2A2A2A]' };
      ...
      <div className="space-y-8">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {cardConfig.map((card) => {
            ...
            return (
              <div key={card.key} className="rounded-3xl border border-[#2A2A2A] bg-[#000000] p-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
          <section className="rounded-3xl border border-[#2A2A2A] bg-[#000000] p-6">
            ...
            {loading ? (
              <div className="space-y-4 animate-pulse">
                {[...Array(3)].map((_, idx) => (
                  <div key={idx} className="h-24 rounded-3xl bg-[#111111]" />
                ))}
              </div>
              ...
              <div className="space-y-4">
                {recentOrders.map((order) => {
                  ...
                  return (
                    <div key={order.id} className="rounded-3xl border border-[#2A2A2A] bg-[#000000] p-4">
          ...
          <aside className="rounded-3xl border border-[#2A2A2A] bg-[#000000] p-6">
            ...
            <div className="space-y-3">
              ...
              <button
                type="button"
                onClick={() => navigate('/admin/pet-breeds?action=create')}
                className="w-full rounded-2xl bg-pink-600 px-4 py-3 text-sm font-semibold text-white hover:bg-pink-500 transition"
              >
                + Add Breed
              </button>
              <button
                type="button"
                onClick={() => navigate('/')}
                className="w-full rounded-2xl border border-[#2A2A2A] bg-[#000000] px-4 py-3 text-sm font-semibold text-zinc-300 hover:bg-[#1a1c22] transition"
              >
                View Shop
              </button>
            </div>
