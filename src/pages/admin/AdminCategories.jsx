...
const AdminCategories = () => {
  ...
  const handleCreateOrUpdate = async (e) => {
    ...
    try {
      if (editingItem) {
        await axios.put(`/categories/${editingItem.id}`, payload);
  ...
  return (
    <AdminLayout title="Categories" subtitle="Manage categories used for products.">
      <div className="space-y-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <p className="text-sm text-zinc-400">Categories ({categories.length})</p>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <button
              type="button"
              onClick={openForm}
              className="inline-flex items-center gap-2 rounded-2xl bg-sky-600 px-4 py-3 text-sm font-semibold text-white hover:bg-sky-500 transition"
            >
            ...
            <tbody>
              {loading ? (
                ...
                filteredCategories.map((category) => (
                  <tr key={category.id} className="border-b border-zinc-800 hover:bg-zinc-950/70 transition-colors">
        ...
        {showForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-3xl max-h-[calc(100vh-4rem)] overflow-y-auto rounded-3xl border border-zinc-800 bg-zinc-950 shadow-2xl p-8">
              ...
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-zinc-500">{editingItem ? 'Edit category' : 'New category'}</p>
                <h3 className="text-2xl font-semibold text-white">{editingItem ? 'Update category' : 'Add category'}</h3>
              </div>
            ...
            <form onSubmit={handleCreateOrUpdate} className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <button
                  type="submit"
                  disabled={submitting}
                  className="inline-flex items-center gap-2 rounded-2xl bg-sky-600 px-5 py-3 text-sm font-semibold text-white hover:bg-sky-500 disabled:opacity-50"
                >
